...
class QueryEngine:
    ...
    def query_clips_by_actor(
        ...
        with sessionmaker(bind=self.engine)() as session:
            clips = (
                session.query(ds.DialogueEntry)
                .join(
                    ds.VoiceOverEntry,
                    ds.VoiceOverEntry.articy_id == ds.DialogueEntry.articy_id,
                )
                ...
                .with_entities(*entities_to_retrieve)
    ...
    def build_training_dataset(self, actor, format="JSON", item_limit=None):
        ...
        if format == "JSON":
            output = [
                ...
                for clip in clips
                if ta.extract_dialogue(clip.raw_dialogue_entry) != ""
            ]
            return output
        else:
            raise ValueError("Unsupported format. Must be one of: JSON")

    def output_training_dataset(
        self,
        actor,
        dataset_name: str,
        output_folder: str,
        ...
        for output in outputs:
            ...
            dataset_output_abs_filepath = os.path.join(
                wav_output_file_abs_directory, output["filename"]
            )
            dataset_output_relative_filepath = os.path.join("wav", output["filename"])
            shutil.copy(output["filepath"], dataset_output_abs_filepath)
        ...
        with open(os.path.join(output_file_directory, "train_filelist.txt"), "w") as f:
            f.write("\n".join(training_set))
        with open(
            os.path.join(output_file_directory, "validation_filelist.txt"), "w"
        ) as f:
            f.write("\n".join(validation_set))
